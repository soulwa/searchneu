...
async function onSendToMessengerButtonClick(
  ...
  if (macros.DEV && !(await elastic.isConnected())) {
    macros.log(
      'In dev mode and Elasticsearch not available. Class watching does not work. To fix this, make sure Elasticsearch is running on your computer and then run "yarn scrape" and "yarn index"'
    );
    return;
  }
...
app.post("/addSection", async (req, res) => {
  ...
  if (notifData) {
    ...
    notifyer.sendFBNotification(
      req.body.senderId,
      `Thanks for signing up for a section in ${notifData.subject} ${notifData.classId} (CRN: ${notifData.crn})!`
    );
  }
...
app.post("/removeSection", async (req, res) => {
  ...
  if (notifData) {
    ...
    notifyer.sendFBNotification(
      req.body.senderId,
      `You have unsubscribed from notifications for a section of ${notifData.subject} ${notifData.classId} (CRN: ${notifData.crn}).`
    );
  }
...
app.post("/addClass", async (req, res) => {
  ...
  if (notifData) {
    ...
    notifyer.sendFBNotification(
      req.body.senderId,
      `You have subscribed to notifications for the class ${notifData.subject} ${notifData.classId}`
    );
  }
